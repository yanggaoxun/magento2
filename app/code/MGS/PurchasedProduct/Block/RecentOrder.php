...
namespace MGS\PurchasedProduct\Block;
...
class RecentOrder extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    public function __construct(
        ...
        $this->_recentConfig = $this->_scopeConfig->getValue( 'purchasedProduct/general', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
    ...
    public function getFakePurchased()
    {
        ...
        if(isset($this->_recentConfig['product_ids']) && $this->_recentConfig['product_ids']){
            ...
            $fakeName = explode(',', $this->_recentConfig['fakename']);
            ...
            foreach ( $producIds as $key => $id ) {
                ...
                $name = isset($fakeName[$key]) ? $fakeName[$key] : $fakeName[array_rand($fakeName)];
                $address = isset($fakeaddress[$key]) ? $fakeaddress[$key]: $fakeaddress[array_rand($fakeaddress)];
                $info['address'] = sprintf(__('%s'), $address);
                $info['name'] = sprintf(__('%s'), $name);
    ...
    public function getPurchased()
    {
        ...
        foreach ($ordercollection as $orderDatamodel) {
            ...
            $info['address'] = sprintf(__('%s'), $city);
            $info['name'] = sprintf(__('%s'), $customerName);
